...
I won't explain it here. Look it up or ask an AI if you're curious.
...
The Termux:API plugin gives Termux admin-level powers over hardware sensors. PC terminals normally natively have these powers, but on Android,
...
It'll take several seconds to find a mirror to download 
	what's needed to get it operational. Now enter some of these lines in Termux to test that the Termux:API works and appreciate some of what it can do:
...
|Can see the minute when a file was last edited without holding on a file to see its "Properties" or "Get info"|❌<br>Need to click on "Properties"|✔️<br>Enter any folder ><br>click on ⋮ > 'Card'|❌<br>Need to click on "Get info"|✔️|
...
|Can remember to open directly in the Termux folder for when opening the file manager app next time|✔️|✔️|❌|❌|
|Can rename the Termux mount 'home' folder internally, e.g. to '~' which is my preference|✔️<br>☰ > hold on 'home' > rename it > 'OK'|❌|❌|❌|

I aim to look at more file manager apps in the future and update this repo if better ones emerge, so please star ⭐ and watch 👁️ this repo to get notified 🔔. 
...
So you need to mount Termux again each time you boot and want to access the Termux files.
This will create duplicate mounts, so just go to: ☰ > hold on the name of any mount > 'Remove'.

Files: Shortcut and FX seem unreliable to me when it comes to editing files, so I would at most use these to do other things like copying files,
...
The .bashrc, .termux/colors.properties and .termux/termux.properties files customise how Termux looks and even acts,
	...
	.bash_history by the way is a file  that Termux makes automatically which records the lines your entered in Termux.

There's a .sh script file attached in this repo in the .shortcuts/tasks subfolder to symbolise where it should be placed.
	Feel free to download and move it to ~/.shortcuts/tasks for testing. This is what it looks like:
...
Now for the last step, scheduling/automating it:

# Step 4 - this is the last and big one, set up Automation:

Unlike the various file manager automating apps, Termux is the only game in town. Termux is the only terminal in Android that's actually powerful.
...
You'll need to grant this permission to whichever Automating app you use.
...
|:-:|:-:|:-:|:-:|:-:|
|Icon|<img src="Icons/Tasker.webp" width="47%" alt="Tasker's icon">|<img src="Icons/MacroDroid.png" width="33%" alt="MacroDroid's icon">|<img src="Icons/Automate.webp" width="79%" alt="Automate's icon">|<img src="Icons/Automation.png" alt="Automation's icon">|
...
```txt
Automation app > ⚙️ Rules > Add Rule > name 'Rule name' something like '🔔 Notification each 20 minutes' > tick 'Rule active' to turn it on ✔️.
	> 'Add trigger' > '⏰ Timeframe' > set both 'Start' and 'End' to 00:00, keep 'entering' ticked and tick all the 7 days of the week so it works 24/7
		around the clock > tick "Repeat every x seconds" > if you want to trigger it each "O'clock" hour (1 O'clock, 2 O'clock, etc), enter 3600 > 'SAVE'.
		Alternatively, enter 900 to trigger every 15 minutes (quarter past, quarter to and half-past),
			or enter 1200 to trigger every 20 minutes (twenty past, twenty to), etc.
		The Automation app very nicely sticks to "round" time so you know exactly when to expect the timed-based rules to trigger, no matter when you boot.
	> 'Add action' > Don't select 'Run script or executable', that requires root. Instead, select 'Start another program'.
		> 'Method to select application': tick 'by action' > 'Select start type': tick either 'by startService()' OR 'by startForegroundService()',
			preferably the latter to maximise reliably so Android doesn't try to stop this to save battery.
		> 'Select app' (don't click that button, I just write this here as a group of instructions):
			> 'Package name': enter 'com.termux'
			> 'Class full name': enter 'com.termux.app.RunCommandService'
			> 'Activity/action name': enter 'com.termux.RUN_COMMAND'
		> 'Add parameters':
			> 'Parameter type': select 'String' > 'Parameter name': enter 'com.termux.RUN_COMMAND_PATH'
			> 'Parameter value': enter '/data/data/com.termux/files/home/.shortcuts/tasks/TermuxAPI_Notification.sh' (change as needed for your script's path)
			> click 'Add Intent pair' > You'll see that the parameters you added are now below,
				and you can hold on them to delete and start them again if needed.
			> Optional, but highly recommended, if you want to execute the script silently in the background instead of it opening a Termux tab:
				> 'Parameter type': keep as 'boolean' > 'Parameter name': enter 'com.termux.RUN_COMMAND_BACKGROUND' > 'Parameter value': enter 'true'
				> click 'Add Intent pair' > 'Save'
	> 'Save Rule' > '🏠 Overview' > if 'Service:' is 'OFF', click on 'OFF' to turn it 'ON' 💚 > 'Control center' > 'Setting' > tick 'Start at system boot' >
		◀ back (navigation button) > 'Export configuration' > create a dedicated folder for this and click 'USE THIS FOLDER'.
	If you want to run a script on boot, instead of using the Termux:Boot plugin, you can do it in automation by cloning the rule you just created. Go to:
	⚙️ Rules > hold on the rule you just created > 'Clone' > hold on the new newly-created clone > 'Edit' > rename it to whatever you like, such as
		'🔔 Notification on boot' > hold on the trigger > you might have to click on the Back navigation button ◀ > 'delete' > 'Add trigger' >
			'⏰ Device starts' > 'Yes' > 'Save Rule'. Alternatively, instead of cloning, you can just create a new rule and add the same Action as before.
```
...
In case you've yet to create this file, create it with this line in Termux:
...
You might want to check in case you accidentally wrote "allow-external-apps=true" there twice.
In case you did, it'll still work, but just pointlessly write those few extra bytes.
...
- The tasks folder allows the Termux:Widget plugin to execute scripts silently in the background,
	but the Automation app needs that extra Indent pair at the end to achieve the same thing.
